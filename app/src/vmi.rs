...
impl Vmi {
    ...
    pub(crate) fn new(domain_name: &str, json_path: &str, socket_path: &str) -> Result<Self> {
        ...
        Ok(Self { handle, paused: AtomicBool::new(false) })
    }
    ...
    pub fn get_struct_offset(&self, struct_name: &str, field_name: &str) -> Result<u64> {
        let s_cstr = CString::new(struct_name).map_err(|_| VmiError::SymbolNotFound(struct_name.into()))?;
        let m_cstr = CString::new(field_name).map_err(|_| VmiError::SymbolNotFound(field_name.into()))?;
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::SymbolNotFound(format!("{}.{}", struct_name, field_name)));
        }
        Ok(offset)
    }
    ...
    pub fn read_8_va(&self, vaddr: u64, pid: u32) -> Result<u8> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed { addr: vaddr, msg: "read_8_va failed".into() });
        }
        Ok(val)
    }
    ...
    pub fn write_8_va(&self, vaddr: u64, pid: u32, val: u8) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed { addr: vaddr, msg: "write_8_va failed".into() });
        }
        Ok(())
    }
    ...
    pub fn read_8_pa(&self, paddr: u64) -> Result<u8> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed { addr: paddr, msg: "read_8_pa failed".into() });
        }
        Ok(val)
    }
    ...
    pub fn read_16_va(&self, vaddr: u64, pid: u32) -> Result<u16> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed{ addr: vaddr, msg: "read_16_va failed".into() });
        }
        Ok(val)
    }
    ...
    pub fn clear_event(&self, event: *mut vmi_event_t) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed{ addr: 0, msg: "failed to clear event".into() });
        }
        Ok(())
    }
    ...
    pub fn events_listen(&self, timeout: u32) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed{ addr: 0, msg: "error listening for events".into() });
        }
        Ok(())
    }
    ...
    pub fn get_vcpureg(&self, reg: u64, vcpu: u32) -> Result<u64> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed{ addr: 0, msg: "failed to get vcpu reg".into() });
        }
        Ok(val)
    }
    ...
    pub fn set_vcpureg(&self, reg: u64, val: u64, vcpu: u32) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed{ addr: 0, msg: "failed to set vcpu reg".into() });
        }
        Ok(())
    }
    ...
    pub fn write_32_va(&self, vaddr: u64, pid: u32, val: u32) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed { addr: vaddr, msg: "write_32_va failed".into() });
        }
        Ok(())
    }
    ...
    pub fn write_64_va(&self, vaddr: u64, pid: u32, val: u64) -> Result<()> {
        ...
        if status != status_VMI_SUCCESS {
            return Err(VmiError::ReadFailed { addr: vaddr, msg: "write_64_va failed".into() });
        }
        Ok(())
    }
...
impl VmiEvent {
    ...
    pub fn set_interrupt(&mut self, intr: u32, gfn: u64, offset: u64) {
        ...
        self.inner.__bindgen_anon_1.interrupt_event.__bindgen_anon_1.__bindgen_anon_1.gfn = gfn;
        self.inner.__bindgen_anon_1.interrupt_event.__bindgen_anon_1.__bindgen_anon_1.offset = offset;
    }
    ...
    pub unsafe fn get_x86_regs(&self) -> *mut crate::ffi::x86_regs {
    ...
    pub unsafe fn set_reinject(&mut self, reinject: i8) {
        self.inner.__bindgen_anon_1.interrupt_event.__bindgen_anon_1.__bindgen_anon_1.reinject = reinject;
    }
...
pub mod event_helpers {
    ...
    pub unsafe fn set_reinject(event: *mut vmi_event_t, val: i8) {
        unsafe {
            (*event).__bindgen_anon_1.interrupt_event.__bindgen_anon_1.__bindgen_anon_1.reinject = val;
        }
    }
...
impl Vmi {
    ...
    pub fn translate_uv2p(&self, dtb: u64, vaddr: u64) -> Result<u64> {
        ...
        let status = unsafe {
            vmi_pagetable_lookup(self.handle, dtb, vaddr, &mut paddr)
        };
        if status == status_VMI_SUCCESS {
            Ok(paddr)
        } else {
            Err(VmiError::ReadFailed { addr: vaddr, msg: "Page table lookup failed".into() })
        }
    }
    ...
    pub fn translate_kv2p(&self, vaddr: u64) -> Result<u64> {
        let mut paddr: addr_t = 0;
        let status = unsafe {
            vmi_translate_kv2p(self.handle, vaddr, &mut paddr)
        };
        if status == status_VMI_SUCCESS {
            Ok(paddr)
        } else {
    ...
    pub fn read_pa(&self, paddr: u64, length: usize) -> Result<Vec<u8>> {
        ...
        let status = unsafe {
            vmi_read_pa(self.handle, paddr, length, buffer.as_mut_ptr() as *mut std::ffi::c_void, &mut read)
        };
        if status == status_VMI_SUCCESS && read == length {
            Ok(buffer)
        } else {
             Err(VmiError::ReadFailed { addr: paddr, msg: "Physical read failed".into() })
        }
    }
    ...
    pub fn read_unicode_string_dtb(&self, dtb: u64, vaddr: u64) -> Result<String> {
        ...
        if length > 4096 { return Ok("<too_long>".into()); }
        ...
        let buf_vaddr = u64::from_le_bytes([
            buf_ptr_raw[0], buf_ptr_raw[1], buf_ptr_raw[2], buf_ptr_raw[3],
            buf_ptr_raw[4], buf_ptr_raw[5], buf_ptr_raw[6], buf_ptr_raw[7]
        ]);
        
        if buf_vaddr == 0 { return Ok(String::new()); }
