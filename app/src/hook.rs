...
use crate::ffi::{
    vmi_event_t, vmi_instance_t, event_response_t,
    VMI_EVENTS_VERSION, INT3, RIP, RSP,
...
impl HookManager {
    pub fn init(vmi: Arc<Mutex<Vmi>>) -> Result<Arc<Self>> {
        ...
        unsafe {
            ...
            (*int_event).set_interrupt(INT3 as u32, 0, 0);
        ...
        F: Fn(&HookContext) + Send + Sync + 'static,
    {
        ...
        if orig_byte == 0xCC {
            return Err(VmiError::Other(format!("intent3 already at {:#x}, previous session may have crashed?", addr)));
        }
        ...
        if let Some(ref s) = strategy {
             eprintln!("[HookManager] Auto-Emulation enabled for {:#x}: {:?}", addr, s);
        } else {
             eprintln!("[HookManager] no emulation for {:#x}, hook is one-shot", addr);
        }
        
        vmi_lock.write_8_va(addr, 0, 0xCC)?;

        state.hooks.insert(addr, Hook {
            addr,
            orig_byte,
    ...
    pub fn shutdown(&self) {
        ...
        if state.hooks.is_empty() { return; }

        eprintln!("[HookManager] restoring {} hooks during shutdown...", state.hooks.len());
    ...
    unsafe extern "C" fn interrupt_cb(
        ...
        unsafe {
            ...
            let rip = match vmi_events.get_vcpureg(RIP as u64, vcpu_id) {
                Ok(r) => r,
                Err(e) => { eprintln!("[HookManager] RIP read failed: {:?}", e); return 0; },
            };
            ...
            if let Some((addr, orig_byte)) = hook_data {
                ...
                if let Some(hook) = state.hooks.get(&rip) {
                    ...
                    if let Some(strategy) = &hook.strategy {
                        match strategy {
                            EmulationStrategy::MoveToMem { src_reg, base_reg, displacement, len } => {
                                let execute_emulation = || -> Result<()> {
                                    ...
                                    vmi_events.write_64_va(target, 0, src_val)?;
                                    (*event_helpers::get_x86_regs(event)).rip = rip + len;
                                    Ok(())
                                };
                                
                                if let Err(e) = execute_emulation() {
                                    eprintln!("[HookManager] emulation failed: {}, removing hook", e);
                            ...
                            EmulationStrategy::MovRegReg { dst_reg, src_reg, len } => {
                                let execute_emulation = || -> Result<()> {
                                    let src_val = vmi_events.get_vcpureg(*src_reg, vcpu_id)?;
                            ...
                            EmulationStrategy::SubImm { reg, imm, len } => {
                                let execute_emulation = || -> Result<()> {
                                    ...
                                    vmi_events.set_vcpureg(*reg, val.wrapping_sub(*imm), vcpu_id)?;
                                    (*event_helpers::get_x86_regs(event)).rip = rip + len;
                                    Ok(())
                                };
                            ...
                            EmulationStrategy::Lea { dst_reg, base_reg, displacement, len } => {
                                let execute_emulation = || -> Result<()> {
                                    let base_val = vmi_events.get_vcpureg(*base_reg, vcpu_id)?;
                         ...
                         eprintln!("[HookManager] no emulation for {:#x}, removing hook (one-shot)", addr);
                         let _ = vmi_events.write_8_va(addr, 0, orig_byte);
                         event_helpers::set_reinject(event, 1);
                    }
                }
